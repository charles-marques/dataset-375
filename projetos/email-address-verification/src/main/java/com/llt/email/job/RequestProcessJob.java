...
public class RequestProcessJob {
	private static final Logger logger = LoggerFactory.getLogger(RequestProcessJob.class);
	...
	public void process() {
		try {
			// 1. verify
			EmailJob emailJob = emailJobDaoImpl.getJob(JobName.JOB_SEND_RESPONSE.getCode());

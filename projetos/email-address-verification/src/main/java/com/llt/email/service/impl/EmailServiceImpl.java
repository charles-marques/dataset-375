...
import com.llt.email.util.EmailAddressBuilder;
...
public class EmailServiceImpl implements EmailService {
    ...
    private static final String TEMPLATE_IDENTIFYING_EMAIL = "email_to_be_identified.ftl";
    ...
    //@Autowired
    private EmailAddressBuilder emailAddressbuilder = new EmailAddressBuilder();
    ...
    public void createEmailRequest(EmailRequest request) {
        //1. insert in to email_request table about the new request
    ...
    protected void processRequestsWithInvalidEmailAddresses() {
        List<EmailRequest> invalidRequests = emailRequestDao.findInvalidEmailRequests(getCutOffTime());

        if (null == invalidRequests || invalidRequests.size() == 0) {
            logger.debug("There are no invalid requests at the cut off time:" + getCutOffTime());
    ...
    protected void processRequestsWithValidEmailAddresses() {
        List<EmailRequest> validRequests = emailRequestDao.findValidEmailRequests(getCutOffTime());

        if (null == validRequests || validRequests.size() == 0) {
            logger.debug("There are no valid requests at the cut off time:" + getCutOffTime());
    ...
    protected void processInvalidEmailRequest(EmailRequest req) {
        ...
        try {
            ...
            notificationService.send(fromAddress, req.getRequestorEmailAddress(),
                invalidSubject(req.getFirstName(), req.getLastName()), attributes, TEMPLATE_INVALID_EMAIL);

            // 2. Update the EmailRequest Status
            emailRequestDao.update(req.getEmailRequestId(), Status.COMPLETED, "");
        } catch (Exception e) {
            logger.error("Unable to send email to " + req.getRequestorEmailAddress());
        }
    }
    ...
    protected void processValidEmailRequest(EmailRequest req) {
        ...
        try {
            // Get the responses for the request which received 9 invalid responses
            // Ideally it should find only one response with the status NEW as
            // this message is not bounced.
            List<EmailResponse> responses = emailResponseDao.findByStatus(req.getEmailRequestId(), Status.NEW);
            ...
            notificationService.send(fromAddress, req.getRequestorEmailAddress(),
                validSubject(req.getFirstName(), req.getLastName()), attributes, TEMPLATE_VALID_EMAIL);

            // 3. Update the EmailRequest Status
            emailRequestDao.update(req.getEmailRequestId(), Status.COMPLETED, validEmailAddress);
        } catch (Exception e) {
            logger.error("Unable to send email to " + req.getRequestorEmailAddress(), e);
        }
    }

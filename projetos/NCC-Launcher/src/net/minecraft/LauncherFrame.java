...
import net.minecraft.resources.Resources;
...
public class LauncherFrame extends Frame
{
  ...
  {
    super("Next Century Craft Launcher");
    /*try
    {
    ...
    try
    {
      setIconImage(ImageIO.read(Resources.class.getResource("favicon.png")));
...
//---------------------------------
  ...
  public void login(String userName, String password) {
    try {
      String parameters = "mail=" + URLEncoder.encode(userName, "UTF-8") + "&password=" + URLEncoder.encode(password, "UTF-8") + "&version=" + 13;
      String result = Util.excutePost("http://login.nextcraft.ru", parameters);
      if (result == null) {
      ...
      loginForm.loginOk();
      loginForm = null;
      setTitle("NextCraft - " + values[2].trim());
    } catch (Exception e) {
      e.printStackTrace();
      showError(e.toString());
      loginForm.setNoNetwork();
  ...
  public static void main(String[] args) {
    ...
    if (args.length >= 1) {
      launcherFrame.loginForm.userMail.setText(args[0]);
      if (args.length >= 2) {

...
public class IdentificaIdiomaRoute extends RouteBuilder {

	public void configure() throws Exception {
		from("file:outbox").transform(body().convertToString()).enrich(
				"direct:identifica-idioma", new PrefixoIdioma()).to("bean:tap");

		// identifica o idioma do texto do objeto da mensagem
		// e grava no header LANGUAGE o idioma
		from("direct:identifica-idioma").transform(body().convertToString())
				.process(new ParametroHttp())
					.to("http://api.langid.net/identify.json").transform(
						body().convertToString()).process(
						new JSONIdentificaLinguaProcessor());
	}

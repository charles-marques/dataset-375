    ...
    throws ServletException, IOException {
        ...
        if(confirmationCode == null || confirmationCode.equals("")) {
            ...
            try {
                ...
                out.println("<link href=\"/yasmim/resources/css/default.css\" rel=\"stylesheet\" type=\"text/css\"/>");
                out.println("</head>");
                out.println("<body>");
                out.println("<h1>Email Confirmation Failure</h1>");
                out.println("<p>This email confirmation is invalid. Check if the address on the browser coincides with the address sent my email.</p>");
                String scheme = request.getScheme();
                String serverName = request.getServerName();
                int serverPort = request.getServerPort();
                String contextPath = request.getContextPath();
                out.println("<p><a href=\""+ scheme + "://" + serverName + (serverPort != 80?":"+ serverPort:"") + (contextPath.equals("")?"":contextPath) +"\">Go to Yasmim website</a>");
                out.println("</body>");
                out.println("</html>");
            } finally {
                out.close();
        ...
        try {
            userAccountBsn.confirmEmail(confirmationCode);
        }
        catch(Exception e) {
            PrintWriter out = response.getWriter();
            try {

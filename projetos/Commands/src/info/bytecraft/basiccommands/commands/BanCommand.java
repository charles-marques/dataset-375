		...
		if (cmd.getName().equalsIgnoreCase("ban") && args.length == 1) {
			...
			if (target != null) {
				if (cs instanceof Player) {
					...
					if (pp.isGuardian() || pp.isAdmin()) {
						Bans bans = plugin.getDatabase().find(Bans.class)
								.where().ieq("playerName", target.getName())
								.findUnique();
						if (bans == null) {
							bans = new Bans();
							...
							Bukkit.broadcastMessage(target.getDisplayName()
									+ ChatColor.RED + " has been banned by "
									+ player.getDisplayName());
							return true;
						} else {
							return true;
					...
					Bans bans = plugin.getDatabase().find(Bans.class).where()
